...
.factory("DataFormat",function(Room,$rootScope){//服务器返回信息格式化，全部校准后可以去掉此部分的循环体，直接返回内容，避免多余的处理
	...
	return LTAction;
}).factory("NJMessage",function(Room,User,Chat,$ionicLoading){
	var jmessage = {
		params:{
			appkey:'',
			timestamp:'',
        ...
        <p class="chat-text positive-bg text-left"> {{chat.chatText}}</p>
			...
			for(var m in t){
				...
				if(t[m].userId != userId){
					...
					html += '<a style="display:block;position:absolute;left:0px;top:0px;height:30px;width:30px;border-radius:15px;overflow:hidden;"><img src="'+t[m].avatar+'"/></a>';
					...
					html += '<div class="item item-avatar-right-s chat-me text-right"> <img src="'+t[m].avatar+'" />';
